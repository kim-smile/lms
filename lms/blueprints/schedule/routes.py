...
@bp.get("/", endpoint="home")  # ✅ 엔드포인트: schedule.home
@login_required
def page():
    uid = g.user.id
    ...
    if q:
        ...
        ev_q = ev_q.filter(
            or_(
                func.lower(CalendarEvent.title).like(like),
                func.lower(CalendarEvent.location).like(like),
                func.lower(CalendarEvent.description).like(like),
            )
        )
    ...
    return render_template(
        "schedule.html",
        items=items, start=start, end=end, q=q_raw, include_assign=include_assign, courses=my_courses
    )
