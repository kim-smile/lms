...
from models import db, User, MentoringTeam, MentoringTeamMember, MentoringReport, Project, ProjectTask, Competition, CompetitionEntry
...
def home():
    ...
    my_projects = (
        db.session.query(Project).filter(or_(*conds)).order_by(Project.created_at.desc()).all()
    )
