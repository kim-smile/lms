...
@bp.get("/", endpoint="home")  # ✅ 엔드포인트: assignments.home
@login_required
def index():
    uid = g.user.id

    # 내가 수강 중인 강좌의 과제 목록 (마감일 오름차순, 마감일 없는 건 뒤로)
    rows = (
    ...
    # 해당 사용자 제출 맵
    assignment_ids = [a.id for (a, _c) in rows]
    subs = (
        db.session.query(Submission)
        .filter(Submission.user_id == uid, Submission.assignment_id.in_(assignment_ids) if assignment_ids else False)
        .all()
    )
...
def submit(aid: int):
    ...
    if file and not file_url:
        ...
        return redirect(url_for("assignments.home"))  # ✅ 엔드포인트 일치
